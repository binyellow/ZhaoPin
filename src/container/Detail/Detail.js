...
export default class Detail extends Component {
  ...
  componentDidMount() {
    ...
    const { userName } = this.props.UserList.userList.find(
  ...
  comment = (content, score) => {
    ...
    addComment({ content, to, score, toName, fromName: userName, type }).then(
      res => {
  ...
  render() {
    ...
    const isCollected = collectionList.some(
      v => v.from === _id && v.to === username
    );
    const userItem = userList.find(v => v._id === username);
    const thisOneCommentList = commentList.filter(v => v.to === username);
    const workingPlace = cityData[0].find(
      item => item.value === userItem.workingPlace
    );
    const myImg = src => (
      <img
        src={require(`../../components/NavLink/img/${src}.png`)}
      ...
      <div>
            ...
            <Popover
              ...
              overlay={[
                <Popover.Item key="6" value="button ct" icon={myImg("comment")}>
                  <span onClick={() => this.handleVisible(true, "yes")}>
                    评论
                  </span>
                </Popover.Item>
              ]}
        ...
        <div>
            ...
            img={
              <img
                ...
                src={require(`../../components/img/${
                  userItem.avatar ? userItem.avatar : "boy"
                }.png`)}
              />
            }
          ...
          <List>
            <Item multipleLine>
              <Brief>
                {userItem.type === "boss" ? "技能要求" : "技术栈"}：
                {userItem.desc}
              </Brief>
              <Brief>
                {userItem.type === "boss" ? "工作经验要求" : "工作经验"}：
                {experienceData[userItem.experience]}
              </Brief>
              <Brief>工作地点：{workingPlace.label}</Brief>
              <Brief>
                {userItem.type === "boss" ? "招聘岗位" : "应聘岗位"}：
                {userItem.title}
              </Brief>
              <Brief>
                {userItem.type === "boss"
                  ? `薪资：${userItem.money}`
                  : `期待薪资：${userItem.expectMoney}`}
              </Brief>
              <Brief>
              ...
              <Brief>
                最后登录时间：
                {this.state.lastLoginTime === ""
                  ? ""
                  : new Date(this.state.lastLoginTime).toLocaleString()}
              </Brief>
            </Item>
          </List>
        </div>
        <Row style={{ margin: "5px 0" }} type="flex" justify="space-around">
          <Col span={7}>
            <Button type="primary" onClick={this.toChat}>
              发送消息
            </Button>
          </Col>
          <Col span={7}>
          ...
          <Col span={7}>
            <Button type="primary" onClick={this.collection}>
              {isCollected ? "取消收藏" : "收藏"}
            </Button>
          </Col>
        </Row>
        <List renderHeader={() => <h4>评价列表：</h4>}>
          {thisOneCommentList.map(item => (
            <Item key={item._id}>
              ...
              <Brief>评分：{item.score}分</Brief>
              <Brief>评价：{item.content}</Brief>
              <Brief>时间：{new Date(item.reviewTime).toLocaleString()}</Brief>
            </Item>
          ))}
        </List>
        <AddRemark
